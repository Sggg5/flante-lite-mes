...
- 进入“补库计算”，使用十二步向导选择一个已完成的销售、库存、水管在制、管件在制和常规产品批次；周计划批次可选。
- 选择六种目标库存算法之一，并按需设置六个月权重、固定目标、订单输入和向上取整规则。创建运行后输入批次、策略和配置会被冻结，来源变化必须新建运行。
- 计算完成后默认仅显示正数建议；可切换全部产品，并按产品、算法、审核状态和问题筛选。先处理阻断问题，再接受、调整或拒绝建议；调整和拒绝必须填写原因。
- 所有正数建议审核完成后单独批准运行，再将 `ACCEPTED` / `ADJUSTED` 且确认量大于 0 的建议转换到“生产需求池”。重复转换是幂等的。
- 生产需求池在本阶段只支持查询和取消尚未分配的需求，不创建周排产、工序、设备任务或报工记录。
- 公式、验收样例和异常代码见 [补库公式 V1](docs/replenishment-formula-v1.md)，实现、测试和人工验收见 [阶段 3 完成报告](docs/phase-3-replenishment-report.md)，性能结果见 [阶段 3 基准](docs/phase-3-replenishment-benchmark.md)。
